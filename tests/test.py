import pytest
import pydantic
from overture_to_osm.process import process_geojson_props, PlaceProps
...
def test_process_geojson_props(props_dict):
    """Test that all properties are processed correctly"""
    new_props = process_geojson_props(props_dict)
    assert new_props == {
        "name": "Primary Name",
        "brand": "Brand Name",
...
def test_process_geojson_props_low_confidence(props_dict):
...
def test_process_geojson_props_invalid_id(props_dict):
    """Test that invalid properties are not processed"""
    del props_dict["id"]
    with pytest.raises(pydantic.ValidationError):
        process_geojson_props(props_dict)


def test_process_geojson_props_invalid_confidence(props_dict):
    props_dict["confidence"] = -0.1
    with pytest.raises(pydantic.ValidationError):
        process_geojson_props(props_dict)


def test_process_geojson_props_invalid_address(props_dict):
    del props_dict["addresses"][0]["freeform"]
    new_props = process_geojson_props(props_dict)
    assert "addr:street_address" not in new_props


def test_process_geojson_props_invalid_phones(props_dict):
...
def test_process_geojson_props_invalid_websites(props_dict):
...
def test_process_geojson_props_invalid_socials(props_dict):
